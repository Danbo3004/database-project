 ...
 *
 * BoundedPriorityQueue<int> bpq(15); // Holds up to fifteen values.
 *
...
class BoundedPriorityQueue {
 public:
  // Constructor: BoundedPriorityQueue(size_t maxSize);
  // Usage: BoundedPriorityQueue<int> bpq(15);
  // --------------------------------------------------
  // Constructs a new, empty BoundedPriorityQueue with
  // maximum size equal to the constructor argument.
  ///
  explicit BoundedPriorityQueue(size_t maxSize);

  // void enqueue(const T& value, double priority);
  // Usage: bpq.enqueue("Hi!", 2.71828);
  // --------------------------------------------------
  // Enqueues a new element into the BoundedPriorityQueue with
  ...
  // --------------------------------------------------
  // Returns the element from the BoundedPriorityQueue with the
  ...
  // std::numeric_limits<double>::infinity() if the queue is
  // empty.
  double best() const;
  double worst() const;

 private:
  // This class is layered on top of a std::multimap mapping from priorities
  // to elements with those priorities.
  std::multimap<double, T> elems;
  size_t maximumSize;
};

/** BoundedPriorityQueue class implementation details */

template <typename T>
BoundedPriorityQueue<T>::BoundedPriorityQueue(size_t maxSize) {
  maximumSize = maxSize;
}
...
template <typename T>
void BoundedPriorityQueue<T>::enqueue(const T& value, double priority) {
...
template <typename T>
T BoundedPriorityQueue<T>::dequeueMin() {
  // Copy the best value.
...
template <typename T>
size_t BoundedPriorityQueue<T>::size() const {
  return elems.size();
}

template <typename T>
bool BoundedPriorityQueue<T>::empty() const {
  return elems.empty();
}

// maxSize just returns the appropriate data member.
template <typename T>
size_t BoundedPriorityQueue<T>::maxSize() const {
  return maximumSize;
}
...
template <typename T>
double BoundedPriorityQueue<T>::best() const {
  return empty() ? std::numeric_limits<double>::infinity() : elems.begin()->first;
}

template <typename T>
double BoundedPriorityQueue<T>::worst() const {
